...
| Preview build Android — `f823789d` (2026-08-04, fixes del pipeline de push notifications: `registerPushToken()` en sesión restaurada, `projectId` explícito, plugin `expo-notifications`, handler de foreground temprano, fix badge "J" tapado en el carnet; hasta el commit `0934eb1`) | ✅ [APK](https://expo.dev/accounts/noisydev/projects/uncas-rugby-app/builds/f823789d-2b37-425b-b2c0-38f21cde8614) |
...
- **Push notifications — auditoría 2026-08-04 (con Opus) + fixes.** Nadie en todo el club tenía un token de push funcionando de forma confiable — en `push_tokens` había una sola fila en toda la tabla (la de Agus, del 30/07, nunca renovada). Causas encontradas y ya corregidas:
  - `registerPushToken()` sólo se llamaba desde el login explícito (`useLogin.ts`) — una sesión restaurada al abrir la app (lo normal, casi nadie desloguea) nunca registraba/renovaba el token. Ahora se centraliza en el listener de auth de `_layout.tsx`, cubre login fresco, biométrico y sesión restaurada.
  - `Notifications.getExpoPushTokenAsync()` no pasaba `projectId` (obligatorio en la versión de expo-notifications del proyecto) — fallaba en silencio (el único `catch` hacía `console.log`, invisible en producción, no hay Sentry/logging remoto en el repo).
  - Faltaba el plugin `expo-notifications` en `app.config.js` (afecta ícono/canal de Android).
  - El handler que muestra notificaciones en foreground (`setNotificationHandler`) sólo se registraba si se montaba la pantalla de login — con sesión restaurada podía no correr nunca. Se movió el import a `_layout.tsx`, que siempre carga primero.
  - **Faltaba la credencial FCM V1 (Google Service Account Key) en EAS** — `eas credentials -p android` mostraba "None assigned yet". Sin esto Expo puede aceptar el envío (200 general) pero Firebase nunca entrega nada. Ya se generó en Firebase Console (Configuración del proyecto → Cuentas de servicio) y se asignó vía `eas credentials -p android` → perfil preview → Push Notifications (FCM V1). Es server-side, no requiere build nueva del mobile.
  - `enviarExpoPush()` en `notifications/index.ts` ahora parsea el array `data` (tickets) de la respuesta de Expo y loguea el detalle de cualquier ticket con `status:'error'` (`DeviceNotRegistered`, `InvalidCredentials`, etc.) — antes sólo revisaba el status HTTP general, que puede ser 200 aunque el mensaje puntual haya fallado. Deployada.
  ...
  - **Pendiente de verificar**: con la build `f823789d` instalada (renueva el token) y la credencial FCM V1 ya puesta, falta confirmar en un test real que la notificación llega al teléfono — la primera prueba post-fix se hizo todavía con la build vieja (`942188b3`, token sin renovar) y no llegó nada.
  - **Inconsistencia menor sin resolver**: `guardarNotificacion()` (inserta en la tabla `notificaciones` para el historial in-app) sólo se llama para `lesion`/`fichaje`/`ausencias_consecutivas` — `noticia_publicada`, `manual` y `cancelacion_entrenamiento` nunca quedan registradas ahí, aunque el push si se intente enviar. No afecta la entrega del push, sólo el historial in-app.
