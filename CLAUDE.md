...
8. **Pendiente:** confirmar con los 4 testers que el build 6 resuelve el freeze de verdad (sobre todo dejando la app cerrada un rato largo antes de reabrir, que es cuando más se disparaba). Hay además una submission completa a App Store (no sólo TestFlight) que Agus mandó sin querer apuntando al build 3 buggy — **cancelada**, pendiente de resubmit recién cuando el fix esté confirmado.
9. **Dev build para iPhone físico — investigado, no ejecutado.** `eas.json` ya tiene el perfil `development` (`developmentClient: true`, `distribution: internal`) y `expo-dev-client` ya está instalado — nada bloqueante. Sólo falta `eas device:create` para registrar el UDID del iPhone de test (flujo sin Mac: genera un link/QR que el dueño del teléfono abre desde el suyo). Ojo: la cuenta de Apple Developer es nueva (~2026-08-10) y Apple puede tardar 24-72h en procesar un dispositivo nuevo — conviene registrar apenas se decida usar este camino, no el mismo día que se quiere buildear. Alternativa más rápida para iterar en JS sin rebuild: el proyecto **no tiene `expo-updates`/`eas update` configurado** — hoy cualquier fix de JS, incluso para producción, exige build nueva.
