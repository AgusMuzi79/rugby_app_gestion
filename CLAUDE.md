...
- **Esperar el resultado de la revisión de Apple** (enviada 2026-08-16, hasta 48hs) — sin acción pendiente salvo reaccionar si rechazan.
...
- **Hallazgo de paso, sin implementar**: `Padron Extendido.xlt.xls` trae la forma de pago real de cada socio (columna `Vendedor`: `VISA`+`Tipo Tarjeta=DÉBITO` = débito automático real, 409 de 1489 socios activos; `CLUB`/`COBRADOR` = paga en el club/con cobrador). Podría usarse para avisarle sólo a esos 409 que el semáforo puede tardar en reflejar su pago — ver [[project-forma-pago-padron]] en memoria, no implementado todavía.

**Próximo paso:**
- Esperar el mail de Apple sobre la revisión completa (build 7, enviada 2026-08-16).
- Seguir el conteo de 12 testers / 14 días corridos de Android (Closed Testing "alpha") hasta poder pedir acceso a Producción.
